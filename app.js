...
// Configuración global para la marca de agua
const watermarkConfig = {
    text: "CONFIDENCIAL", // Texto por defecto, será sobrescrito por el inspector
    fontSize: 55,
    color: [128, 128, 128], // RGB para gris
    opacity: 0.4, // Opacidad deseada
    angle: 45, // Ángulo de rotación en grados
    xPosition: '150', // 'center', 'left', 'right' o un valor numérico en mm
    yPosition: 'center', // 'center', 'top', 'bottom' o un valor numérico en mm
    fontFamily: 'Helvetica', // Fuente de la marca de agua
    renderingMode: 'fill' // 'fill' o 'stroke'
};

// URL de tu Google Apps Script Web App
// ¡IMPORTANTE! Reemplaza esta URL con la que obtuviste al desplegar tu Apps Script.
const GOOGLE_APPS_SCRIPT_URL = "https://script.google.com/macros/s/AKfycbx8Av1soxLz-HH4czTCCKu5l-F8NcjGlGRazblLJOkiIUBwqvEIwWFSx6kb-PQtVaw/exec"; 
...
function handlePhotoUpload(inputElement, previewIdPrefix) {
    const file = inputElement.files[0];
    if (file) {
        const reader = new FileReader();
        reader.onload = function(e) {
            // Extraer el ID numérico del input
            const idParts = inputElement.id.split('-');
            const photoId = idParts[idParts.length - 1]; // Obtener el último segmento del ID

            const preview = document.getElementById(`${previewIdPrefix}${photoId}`);
            if (preview) { // Verificar que el elemento de vista previa existe
                preview.src = e.target.result;
                preview.style.display = 'block';
            } else {
                console.error(`Error: No se encontró el elemento de vista previa con ID: ${previewIdPrefix}${photoId}`);
            }
        };
...
document.getElementById('add-photo-btn').addEventListener('click', function() {
    ...
    document.getElementById(`photo-upload-${photoCountConstatacionModelo}`).addEventListener('change', function() {
        handlePhotoUpload(this, 'photo-preview-'); // Pasa 'this'
    });
});
...
document.getElementById('add-photo-btn-verificacion').addEventListener('click', function() {
    ...
    document.getElementById(`photo-upload-verificacion-${photoCountVerificacion}`).addEventListener('change', function() {
        handlePhotoUpload(this, 'photo-preview-verificacion-'); // Pasa 'this'
    });
});
...
document.getElementById('add-photo-btn-s-signos').addEventListener('click', function() {
    ...
    document.getElementById(`photo-upload-s-signos-${photoCountSSignos}`).addEventListener('change', function() {
        handlePhotoUpload(this, 'photo-preview-s-signos-'); // Pasa 'this'
    });
});
...
document.getElementById('add-photo-btn-deposito').addEventListener('click', function() {
    ...
    document.getElementById(`photo-upload-deposito-${photoCountDeposito}`).addEventListener('change', function() {
        handlePhotoUpload(this, 'photo-preview-deposito-'); // Pasa 'this'
    });
});
...
function applyWatermark(doc, inspector) {
    // Usar las propiedades del objeto watermarkConfig global
    doc.setFontSize(watermarkConfig.fontSize);
    doc.setFont(watermarkConfig.fontFamily, 'bold'); // Establecer la fuente en negrita

    // Crear un estado gráfico para la opacidad
    const gState = new window.jspdf.GState({
        opacity: watermarkConfig.opacity
    });
    doc.setGState(gState); // Aplicar el estado gráfico

    // Establecer el color del texto (sin opacidad aquí, ya que se maneja con gState)
    doc.setTextColor(
    ...
    if (watermarkConfig.xPosition === 'center') {
        ...
        xPos = parseFloat(watermarkConfig.xPosition); // Asegurarse de que sea un número
    }
    ...
    if (watermarkConfig.yPosition === 'center') {
        ...
        yPos = parseFloat(watermarkConfig.yPosition); // Asegurarse de que sea un número
    }

    doc.text(
        inspector.toUpperCase(), // El texto de la marca de agua sigue siendo el inspector
        xPos,
        yPos, {
            align: 'center',
    ...
    // Restablecer el estado gráfico a uno sin opacidad para no afectar otros elementos
    doc.setGState(new window.jspdf.GState({
        opacity: 1
    }));
    doc.setTextColor(0, 0, 0); // Restablecer color a negro
    doc.setFont('helvetica'); // Restablecer la fuente por defecto de jspdf
}
...
async function addPhotoWithWatermark(doc, imgData, inspector, isLastPage = false, signatureImg = null) {
    ...
    if (isLastPage && signatureImg) {
        ...
        doc.setFontSize(10);
        doc.text("FIRMA DEL INSPECTOR", pageWidth / 2, pageHeight - margin - sigHeight - 15, {
            align: 'center'
        });
    }
}

// Función para enviar el PDF a Google Drive
async function sendPdfToGoogleDrive(pdfDoc, fileName) {
    try {
        ...
        await new Promise((resolve, reject) => {
            reader.onloadend = async function() {
                ...
                Swal.fire({
                    ...
                    html: `
                        <b>${fileName}</b><br><br>
                        Se ha guardado correctamente en Google Drive.
                    `,
                    ...
                    backdrop: `
                        rgba(0,0,0,0.5)
                        url("https://i.giphy.com/media/v1.Y2lkPTc5MGI3NjExcDl6b3Z6Y2pnb2tzZDY4eG5veTR4a2lpY2JwYzF1azkyZHBiZXFldCZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9Zw/xT5LMHxhOfscxPfIfm/giphy.gif")
                        center top
                        no-repeat
                    `,
                    timer: 5000,
                ...
                console.log('Acta enviada a Drive:', fileName);
                resolve();
            };
        ...
        Swal.fire({
            icon: 'error',
            title: '❌ Error',
            text: 'No se pudo enviar el acta a Google Drive. Por favor, inténtalo nuevamente.',
...
document.getElementById('generate-btn').addEventListener('click', async function() {
    ...
    doc.save(pdfFileName); // Guarda el PDF localmente
    await sendPdfToGoogleDrive(doc, pdfFileName); // Envía el PDF a Google Drive
});
